var DetailedView = (function(){
    ...
    var svgClass = 'urank-ranking-svg',
        ...
        barClassDocumentLength = barClass + "-document-length";
    // Id
    var stackedbarPrefix = '#urank-ranking-stackedbar-';

    function DetailedView(arguments) {
        _this = this;
        s = $.extend({
    ...
    RANKING.Render = {
        ...
        * ***************************************************************************************************************/
        ...
        /******************************************************************************************************************
        *
        *	Draw positions within the document and bar for document length or update methods. Animate with width transition
        *
        * ***************************************************************************************************************/
        drawStackedBars: function(){
            ...
            setTimeout(function(){
                ...
                var arrayPositionInformation = []
                var keywords = [];

                for(var i = 0; i < data.length; i++) {
                    data[i].weightedKeywords.forEach(function(entry) { keywords.pushIfNotExist(entry.stem); })
                    arrayPositionInformationEntry = {descriptionLength: data[i].description.length, position: getPositionArray(data[i].description, keywords, 0) };
                    arrayPositionInformation.push(arrayPositionInformationEntry);
                }

                var numberOfKeywords = keywords.length;
                var maxDocumentLength = Math.max.apply(Math,arrayPositionInformation.map(function(d){return d.descriptionLength;}));

                for(var j = 0; j < arrayPositionInformation.length; j++) {
                    svg.select(stackedbarPrefix + data[j].id).selectAll("circles")
                        .data(arrayPositionInformation[j].position)
                        .enter()
                        .append("circle")
                        .attr("cx", function (d, i) {
                            var positionX = width * arrayPositionInformation[j].position[i].position / maxDocumentLength;
                            if(positionX < 4)
                                return 4;
                            if(positionX > width * arrayPositionInformation[j].descriptionLength / maxDocumentLength - 4)
                                return width * arrayPositionInformation[j].descriptionLength / maxDocumentLength - 4;
                            return positionX; })
                        .attr("cy", function(d, i) {
                            var keywordIndex = keywords.indexOf(arrayPositionInformation[j].position[i].keyword);
                            return (keywordIndex + 1) * y.rangeBand() / (numberOfKeywords + 1); })
                        .attr("r", 0)
                        .style("fill", function(d, i) { return color(arrayPositionInformation[j].position[i].keyword); })
                        .transition()
                        .duration(500)
                        .attr("r", 3);

                    svg.select(stackedbarPrefix + data[j].id)
                        .append('rect')
                        .attr("class", barClassDocumentLength)
                        .attr('x', 0)
                        .attr('width', 0)
                        .attr('height', y.rangeBand())
                        .attr("fill", "black")
                        ...
                        .transition()
                        .duration(500)
                        .attr({ "width": width * arrayPositionInformation[j].descriptionLength / maxDocumentLength });
                }
            }, 800);
    ...
    var _hoverItem = function(id) {
        if(this.isRankingDrawn) {
            ...
            svg.select(stackedbarPrefix +''+ id).selectAll('.' + barClassDocumentLength)
                .attr('transform', 'translate(0, 0)')
                .style('filter', 'url(#drop-shadow)');
        }
        return this;
