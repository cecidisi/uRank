...
var RankingModel = (function(){
    ...
    *******************************************/
    ...
    var updateRanking =  function(opt){
        var score = opt.mode;
        var totRWeight = 0.0;
        Object.keys(opt.rWeights).forEach(function(r){ totRWeight += opt.rWeights[r]; });
        Object.keys(opt.rWeights).forEach(function(r){ parseFloat(opt.rWeights[r] /= totRWeight); });
        ...
        if(opt.ranking.content)
            ranking = _this.cbRS.getCBScores({ data: ranking, keywords: opt.query, options: { rWeight: opt.rWeights.cb } });
        if(opt.ranking.social)
            ranking = _this.tuRS.getTagUserScores({ user: opt.user, keywords: opt.query, data: ranking, options: { tWeight: opt.rWeights.t, uWeight: opt.rWeights.u } });
        ranking.forEach(function(d){
            d.ranking.overallScore = 0;
            if(opt.ranking.content)
                d.ranking.overallScore += d.ranking.cbScore;
            if(opt.ranking.social) {
                d.ranking.overallScore += d.ranking.tScore;
                d.ranking.overallScore += d.ranking.uScore
            }
        });
        ...
        ranking = ranking.sort(function(d1, d2){
            ...
            //if(d1.ranking[secScore] && d1.ranking[secScore] < d2.ranking[secScore]) return 1;
            if(d1.ranking.overallScore > d2.ranking.overallScore) return -1;
            if(d1.ranking.overallScore < d2.ranking.overallScore) return 1;
            return 0;
        });
 ...
 ****************************************************************************************************/
    RankingModel.prototype = {
        ...
        setData: function(data, traininData) {
            ...
            _this.tuRS.init(_this.trainingData);
            return this;
        },
